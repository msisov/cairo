			...
			cairo_clip_t *clip)
{
    ...
    /* Lazily create the glyph mask */
    /* Delete previous glyph mask if it is too small */
    if(ctx->glyph_mask &&
    		(
    			((cairo_gl_surface_t *)ctx->glyph_mask)->width < info->extents.width ||
    			((cairo_gl_surface_t *)ctx->glyph_mask)->height < info->extents.height )
    		)
    {
    ...
    if(!ctx->glyph_mask)
    {
        ...
        ctx->glyph_mask = cairo_gl_surface_create (dst->base.device,
                                        CAIRO_CONTENT_COLOR_ALPHA,
                                        info->extents.width,
                                        info->extents.height);
        if (unlikely (ctx->glyph_mask->status))
        {
        	status = ctx->glyph_mask->status;
        	status = _cairo_gl_context_release(ctx, status);
            return status;
        }
    ...
    else
    {
    	...
    	_cairo_gl_surface_clear_with_extent(ctx, (cairo_gl_surface_t *)ctx->glyph_mask,
                                            &info->extents, CAIRO_COLOR_TRANSPARENT, FALSE);
    }
 
    status = render_glyphs ((cairo_gl_surface_t *) ctx->glyph_mask,
			    info->extents.x, info->extents.y,
	...
	ctx->glyph_mask->is_clear = FALSE;
	_cairo_pattern_init_for_surface (&mask_pattern, ctx->glyph_mask);
