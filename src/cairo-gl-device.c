...
void
_cairo_gl_context_switch_destination_sampling (cairo_gl_context_t *ctx,
					       cairo_gl_surface_t *surface,
					       cairo_bool_t multisampling)
{
    /* OpenGL ES surfaces are always in MSAA mode once it's been turned on
     * so we cannot switch back and forth. */
    if (ctx->gl_flavor == CAIRO_GL_FLAVOR_ES) {
                                   ...
                                   cairo_bool_t multisampling)
{
    ...
    if (! changing_surface) {
	_cairo_gl_context_switch_destination_sampling (ctx, surface,
						       multisampling);
	return;
    }
    ...
    if (_cairo_gl_surface_is_texture (surface)) {
	/* For Desktop GL we bind the framebuffer while setting up sampling. */
	if (ctx->gl_flavor == CAIRO_GL_FLAVOR_ES) {
	    _cairo_gl_ensure_framebuffer (ctx, surface);
	    ctx->dispatch.BindFramebuffer (GL_FRAMEBUFFER, surface->fb);
	} else {
